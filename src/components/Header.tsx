...
export default function Header() {
  ...
  const navLinks = [
    { href: '/', label: 'Inicio' },
    { href: '/reemplazo-llave-carro-el-paso/', label: 'Servicios' },
  ...
  return (
    <header className="bg-primary/95 backdrop-blur-md text-white shadow-2xl sticky top-0 z-50 border-b border-secondary/20">
      ...
      <div className="hidden md:block bg-secondary/10 border-b border-secondary/20">
        <div className="container mx-auto px-4 py-1.5 flex items-center justify-between text-sm">
          <div className="flex items-center gap-6 text-gray-300">
            ...
            <span>El Paso y Áreas Cercanas</span>
          </div>
          <div className="flex items-center gap-4">
            <a href={`tel:${businessInfoEs.phoneRaw}`} className="text-secondary hover:text-highlight transition font-medium">
        ...
        <div className="flex items-center justify-between py-3">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            <div>
              ...
              <div className="text-[10px] text-gray-400 uppercase tracking-[0.2em]">Cerrajero Automotriz Móvil</div>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-2">
            <a
              href={`sms:${businessInfoEs.textPhone}`}
            ...
            <a
              href={`tel:${businessInfoEs.phoneRaw}`}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-primary/95 backdrop-blur-md border-t border-secondary/20 p-3 flex gap-2">
        <a
          href={`tel:${businessInfoEs.phoneRaw}`}
          className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-secondary to-highlight text-primary py-3.5 rounded-xl font-bold text-base transition"
        >
          <Phone className="w-5 h-5" />
          Llamar Ahora
        </a>
        <a
          href={`sms:${businessInfoEs.textPhone}`}
          className="flex items-center justify-center gap-2 bg-white/10 text-white px-5 py-3.5 rounded-xl font-bold transition border border-white/10"
        >
          <MessageCircle className="w-5 h-5" />
        </a>
      </div>
